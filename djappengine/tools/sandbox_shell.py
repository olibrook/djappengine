...
# Initialize this script as though it were 'dev_appserver'. This needs to come before the
# `dev_appserver` imports below.
...
def appengine_environ():
    """Set up and tear down an environment as though running an application through dev_appserver.

    Usage example:

    >>> import code
    >>> with appengine_environ():
    ...     code.interact()
...
def main():
    """Start an interactive shell in an App Engine environment."""
    with appengine_environ():
        code.interact()
